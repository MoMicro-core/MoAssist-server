...
const assertAllowedUrl = (url, allowedHosts, allowPrivateHosts) => {
  if (url.protocol !== 'https:' && url.protocol !== 'http:') {
    throw new Error(`Protocol "${url.protocol}" is not allowed`);
  }
  // The private-range guard can only be lifted via the dev-only
  // CONNECTOR_ALLOW_PRIVATE_HOSTS flag (local testing against localhost).
...
const createHttp = ({
  ...
  return async (input, init = {}) => {
    ...
    for (let hop = 0; hop <= MAX_REDIRECTS; hop += 1) {
      assertAllowedUrl(url, hosts, allowPrivateHosts);
